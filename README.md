...
| `position` | String | You can set the `position` of the button, the default position is `right`. Other options are `left`, `bottom-left`, `bottom-right` and `custom`. |
...
| `toggleButtonId`| String | Instead of using the default button you can specify any DOM element to toggle the form. Pass the id to `toggleButtonId` to use a custom button. Additionally you can hide the default button by adding `position: 'custom'`. |
